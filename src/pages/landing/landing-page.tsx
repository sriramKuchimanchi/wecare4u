...
import { Accessibility, Activity, Ambulance, ArrowRight, Bell, Bot, Brain, Bug, Building2, Calendar, Car, CheckCircle2, ChevronDown, ChevronRight, Clock, Droplet, Dumbbell, FileText, FlaskConical, Heart, HeartHandshake, HeartPulse, Home, Lock, MapPin, MessageCircle, MessageSquare, Pill, Scan, ShieldAlert, ShieldCheck, Siren, Sparkles, Stethoscope, TestTube, Users, Video, Wrench, Zap, type LucideIcon } from '@/config/icons';
import { Button } from '@/components/ui/button';
import { APP_NAME, APP_TAGLINE } from '@/constants';

const serviceCategories: [LucideIcon, string, string, [LucideIcon, string][]][] = [
  [Building2, 'Healthcare', 'Hospitals, clinics & specialists', [
    [Building2, 'Hospitals'],
    [Stethoscope, 'Clinics'],
    [HeartHandshake, 'Family physicians'],
    [Brain, 'Specialists'],
  ]],
  [FlaskConical, 'Diagnostics', 'Tests & sample collection', [
    [TestTube, 'Blood tests'],
    [Scan, 'X-rays'],
    [HeartPulse, 'ECG'],
    [Home, 'Home sample collection'],
  ]],
  [Pill, 'Pharmacy', 'Medicines & prescriptions', [
    [Pill, 'Medicine delivery'],
    [FileText, 'Prescription management'],
  ]],
  [Activity, 'Home care', 'Skilled support at home', [
    [Activity, 'Nurses'],
    [Dumbbell, 'Physiotherapists'],
    [Users, 'Care attendants'],
  ]],
  [Wrench, 'Home services', 'Everyday household help', [
    [Zap, 'Electricians'],
    [Droplet, 'Plumbers'],
    [Wrench, 'Appliance repair'],
    [Sparkles, 'House cleaning'],
    [Bug, 'Pest control'],
  ]],
  [Car, 'Transportation', 'Getting there, safely', [
    [Ambulance, 'Ambulance'],
    [Car, 'Taxi partners'],
    [Accessibility, 'Wheelchair transport'],
  ]],
  [Siren, 'Emergency contacts', 'Help close to home', [
    [Users, 'Local relatives'],
    [Home, 'Neighbors'],
    [Building2, 'Apartment association'],
    [ShieldAlert, 'Police (where appropriate)'],
  ]],
];
  ...
  <div className="landing-page overflow-hidden bg-[#fffaf4] text-[#10243e]">
    ...
    <section id="services" className="container py-16 md:py-20">
      <div className="mb-9 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <p className="max-w-sm text-sm leading-6 text-[#718096]">Hover any category to see what's inside &mdash; from clinical care to the everyday help around the house.</p>
      </div>
      <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-4">
        {serviceCategories.map(([Icon, title, subtitle, items]) => (
          <div key={title} tabIndex={0} className="group relative outline-none">
            <div className="relative z-10 flex items-center gap-3 rounded-xl border border-[#eadfd4] bg-white px-3.5 py-3 transition-all duration-300 group-hover:border-secondary/50 group-hover:shadow-[0_14px_30px_-18px_rgba(16,36,62,0.35)] group-focus-within:border-secondary/50 group-focus-within:shadow-[0_14px_30px_-18px_rgba(16,36,62,0.35)]">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="truncate text-[0.85rem] font-semibold leading-tight">{title}</h3>
                <p className="mt-0.5 truncate text-[11px] leading-tight text-[#94a3b3]">{subtitle}</p>
              </div>
            ...
            <div className="invisible absolute inset-x-0 top-full z-30 pt-2 opacity-0 transition-all duration-200 ease-out group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100">
              <div className="overflow-hidden rounded-xl border border-white/10 bg-[#10243e] shadow-2xl shadow-black/30 ring-1 ring-secondary/20">
                ...
                <ul className="divide-y divide-white/5 p-1.5">
                  {items.map(([ItemIcon, name]) => (
                    <li key={name} className="flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-xs text-white/80 transition-colors hover:bg-white/5 hover:text-white">
